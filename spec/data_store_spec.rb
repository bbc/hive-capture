...
RSpec.describe 'HiveCapture::DataStore' do
  ...
  describe 'device' do
    it 'creates an entry for an unknown device' do
      expect(HiveCapture::DataStore.device(1)).to eq({})
    end

    it 'sets a value for a new device' do
      HiveCapture::DataStore.device(1)[:key] = 'value'
      expect(HiveCapture::DataStore.instance_variable_get(:@devices)).to match({1 => {key: 'value'}})
    end

    it 'gets an entry for a known device' do
      HiveCapture::DataStore.device(1)[:key] = 'value'
      expect(HiveCapture::DataStore.device(1)).to eq({key: 'value'})
    end

    it 'sets a value for a known device' do
      HiveCapture::DataStore.device(1)[:key] = 'value'
      HiveCapture::DataStore.device(1)[:key2] = 'value2'
      expect(HiveCapture::DataStore.instance_variable_get(:@devices)).to match({1 => {key: 'value', key2: 'value2'}})
    end
    ...
    it 'sets a value for two separate devices' do
      ...
      expect(HiveCapture::DataStore.device(1)).to eq({key1: 'value1'})
      expect(HiveCapture::DataStore.device(2)).to eq({key2: 'value2'})
    end
  end
  ...
  describe 'poll_delay' do
    it 'sets the delay time for a device' do
      HiveCapture::DataStore.poll_delay(1, 5.1)
      expect(HiveCapture::DataStore.instance_variable_get(:@devices)).to match({1 => {delay: 5.1}})
    end

    it 'updates the delay time for a device' do
      HiveCapture::DataStore.poll_delay(1, 5.1)
      HiveCapture::DataStore.poll_delay(1, 2.3)
      expect(HiveCapture::DataStore.instance_variable_get(:@devices)).to match({1 => {delay: 2.3}})
    end
  end
end
